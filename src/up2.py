...
_, lidar = sim.simxGetObjectHandle(clientID, 'Proximity_sensor', sim.simx_opmode_blocking)
...
armJoints = [-1,-1,-1,-1,-1]
for i in range(5):
    _, armJoints[i] = sim.simxGetObjectHandle(clientID,'youBotArmJoint'+str(i+1),sim.simx_opmode_blocking)
...
# Forward Kinematics
for i in range(5):
    _ = sim.simxSetJointPosition(clientID, armJoints[i], desiredJ[i], sim.simx_opmode_streaming)

# Inverse Kinematics to pick up object
_, T = sim.simxGetJointMatrix(clientID, armJoints[0],sim.simx_opmode_streaming)
print(T)

gripperCommunicationTube = sim.simxtubeOpen(0,'youBotGripperState',1)
gripperReturn = im.simTubeWrite(gripperCommunicationTube, sim.packInt32Table({0}), 32)
...
sim.simxSetJointTargetVelocity(clientID, wheelJoints[1], -0.7, sim.simx_opmode_streaming)
sim.simxSetJointTargetVelocity(clientID, wheelJoints[2], -0.7, sim.simx_opmode_streaming)
sim.simxSetJointTargetVelocity(clientID, wheelJoints[3], -0.7, sim.simx_opmode_streaming)

# Stop when sense object
while(1):
    
    _, detectionState, detectionPoint, detectedObjectHandle, detectedSurfaceNormalVector = sim.simxReadProximitySensor(clientID,lidar,sim.simx_opmode_streaming)
    
    if detectionState==False:
        distance = maxDetection
    else:
        #_, distanceHandle = sim.simxGetDistanceHandle(clientID, detectedObjectHandle, sim.simx_opmode_blocking)
        _, distance = sim.simxReadDistance(clientID, detectedObjectHandle, sim.simx_opmode_streaming)
    print(distance)
    #print(detectionState, minDetection,distance)
    if distance < minDetection:
        sim.simxSetJointTargetVelocity(clientID, wheelJoints[0], -0.0, sim.simx_opmode_streaming)
        sim.simxSetJointTargetVelocity(clientID, wheelJoints[1], -0.0, sim.simx_opmode_streaming)
        sim.simxSetJointTargetVelocity(clientID, wheelJoints[2], -0.0, sim.simx_opmode_streaming)
        sim.simxSetJointTargetVelocity(clientID, wheelJoints[3], -0.0, sim.simx_opmode_streaming)
        break
"""
